...
| Local release gate | Verified | `npm run release:gate` runs the local no-PHI proof stack: status blockers, all local phase gates, equity metrics gate, billing artifact gate, accessibility gate, coverage logistics gate, discharge explainer gate, navigator enrollment gate, grant reporting gate, Appendix B residual gate, static preview smoke, and the full test suite. It intentionally excludes public preview verification. |
| Public preview preflight | Built | `npm run preview:preflight` checks clean tree, upstream, pushed commits, no-PHI flag posture, local release proof rung, and Vercel project link before any public preview verification attempt. Current checkout is not yet pushed or Vercel-linked. |
| Stakeholder packet | Available | `npm run release:packet` prints the current commit, push status, proof rung, demo scope, prototype-deferred health-information gates, Appendix B residual counts, missing public receipt, and next commands. |
...
| Spec residuals | `specResiduals` | Appendix B medium, cross-cutting, named-capability, and consistency residuals are tracked separately from demo blockers. |
...
| Local accessibility acceptance works | `npm run accessibility:gate` passes and the accessibility result note is recorded; production WCAG audit and owner sign-off remain blocked. |
...
| Local navigator enrollment demo works | `npm run enrollment:gate` passes and the navigator enrollment result note is recorded; real identity proofing and account creation remain blocked. |
...
| Appendix B residual tracking is current | `npm run spec:gate` passes: Appendix B.3 medium controls, B.4 cross-cutting subsystems, B.5 named brief demo paths, and B.6 right-to-erasure are tracked without becoming stakeholder-demo blockers. |
...
| Local release gate works | `npm run release:gate` passes: `ops:status -- --blockers`, all local phase gates, `npm run equity:gate`, `npm run billing:gate`, `npm run accessibility:gate`, `npm run coverage:gate`, `npm run explainer:gate`, `npm run enrollment:gate`, `npm run grant:gate`, `npm run spec:gate`, `npm run preview:gate`, and `npm test`; `preview:verify` is excluded until a public deployment exists. |
| Stakeholder handoff packet is current | `npm run release:packet` prints a packet whose push status, commit, public-preview receipt state, latest receipt URL/deployment/commit when recorded, proof rung, Appendix B residual counts, and blocker split match the current repo state. |
...
| Preview preflight ready | `npm run preview:preflight` passes: local release rung is verified, `RHTP_REAL_PHI` is off/unset, working tree is clean, branch has an upstream, no local commits are ahead of upstream, and `.vercel/project.json` is present. |
