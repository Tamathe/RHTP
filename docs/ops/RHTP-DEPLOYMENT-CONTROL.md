...
| P2 | Server session plumbing verified | Grounding verifier, crisis recall floor, model-backstop rule-gap ticketing, degraded-model ops alert state, and server-gated Realtime client-secret minting exist locally. Browser WebRTC attach, transcript storage, and the Sandy tool gateway are not built. |
