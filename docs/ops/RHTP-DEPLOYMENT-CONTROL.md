...
| P2 | Browser attach verified | Grounding verifier, crisis recall floor, model-backstop rule-gap ticketing, degraded-model ops alert state, server-gated Realtime client-secret minting, and browser WebRTC attach code exist locally behind off-by-default flags. Transcript storage and the Sandy tool gateway are not built. |
